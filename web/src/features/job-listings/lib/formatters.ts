...
export function formatJobLocationRequirement(
  jobLocationRequirement: JobLocationRequirement,
) {
  switch (jobLocationRequirement) {
    ...
    default:
      throw new Error(
        `Unkown job location requirement: ${jobLocationRequirement satisfies never}`,
      );
  }
}
...
export function formatExperienceLevel(experienceLevel: ExperienceLevel) {
  switch (experienceLevel) {
    ...
    default:
      throw new Error(
        `Unkown experience level: ${experienceLevel satisfies never}`,
      );
  }
}
...
export function formatWage(wageInPaise: number, wageInterval: WageInterval) {
