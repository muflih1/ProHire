...
function PricingRoute() {
  const [planPeriod, setPlanPeriod] = useState<'monthly' | 'annual'>('annual');
  const trpc = useTRPC();
            ...
            <div className='grid w-full gap-1 grid-rows-subgrid row-[span_3] p-4 items-start bg-secondary'>
              ...
              <div className='flex items-center gap-0.5 mt-0.5'>
                <p className='text-2xl font-bold text-primary'>
                  {planPeriod === 'monthly' || plan.annualMonthlyAmount == null
                    ? formatter.format(plan.amount / 100)
                    : formatter.format(plan.annualMonthlyAmount / 100)}
                </p>
              ...
              {plan.annualAmount == null ? (
                ...
                <div className='mt-1'>
                  <label className='flex flex-row items-center justify-start w-fit'>
                    <Switch
                      value={planPeriod}
                      checked={planPeriod === 'annual'}
                      onCheckedChange={checked =>
                        setPlanPeriod(checked ? 'annual' : 'monthly')
                      }
                    />
